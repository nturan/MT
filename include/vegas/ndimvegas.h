...
extern "C" void rlxd_init(int level,int seed);
...
class Vegas {
 ...
 Vegas(Integrand & f, int flen=0) : 
  ndim(f.getDimension()), alpha(0.5), ncall(f.VegasPoints()),
    itmx(f.VegasIterations()), nprn(0), ndo(1), mds(1), 
    acc(f.VegasAcc()), integ(f) {
    //    acc=integ.VegasAcc();
  ...
  void setncall(int i){ncall = i;}
  
  void setitmx(int i){itmx = i;}  
  
  inline int getcalls(){ return(static_cast<int>(calls)); }
